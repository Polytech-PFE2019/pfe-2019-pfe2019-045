    ...
    for epoch in range(epochs):  # for each epoch
        ...
        for step in range(steps_per_epoch):  # for every batch
            ...
            with tf.GradientTape() as g:
                ...
                x = preprocess(x, mode=preprocess_mode, xception=False)
                if np.any(np.isnan(x)):
                    print("nan existed\n")
                [x, y, mask] = convert_to_tensors([x, y, mask])

                x_val, y_val, mask_val = loader.get_batch(size=batch_size, train=True, augmenter=augmenter)

                x_val = preprocess(x_val, mode=preprocess_mode)
                if np.any(np.isnan(x_val)):
                    print("nan existed\n")
                [x_val, y_val, mask_val] = convert_to_tensors([x_val, y_val, mask_val])
            ...
            #     y_ = model.get_layer("final_conv").output
            #
            #
            #
            #
            #     loss = tf.losses.softmax_cross_entropy(y, y, weights=mask)  # compute loss
            #     loss_aux = tf.losses.softmax_cross_entropy(y, y_, weights=mask)  # compute loss
        ...
        if evaluation:
            ...
            print('Train miou: ' + str(train_miou))            # get metrics
            #train_acc, train_miou = get_metrics(loader, model, loader.n_classes, train=True, preprocess_mode=preprocess_mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", help="Model path", default='weight/model')
    ...
    parser.add_argument("--width", help="number of epochs to train", default=224)
    ...
    train_acc, train_miou = get_metrics(loader, model, loader.n_classes, train=True, preprocess_mode=None)
    print('Train accuracy: ' + str(train_acc.numpy()))
    print('Train miou: ' + str(train_miou))
