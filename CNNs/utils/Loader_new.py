...
class Loader:
    def __init__(self, dataFolderPath, width=224, height=224, channels=3, n_classes=21, problemType='segmentation',
        ...
        ########
        self.output_list = [file for file in files if '/output/' in file]
        ...
        if problemType == 'classification':
            ...
            classes_output = [file.split('/output/')[1].split('/')[0] for file in self.output_list]
            classes = np.unique(np.concatenate((classes_train, classes_test, classes_output)))
            self.classes = {}
            ...
            self.image_output_list = [file for file in self.output_list if '/output/' in file]
            ...
            print('Loaded ' + str(len(self.image_output_list)) + ' output samples')
    ...
    def suffle_segmentation(self):
        if self.problemType == 'segmentation':
            s = np.arange(len(self.label_train_list))
            np.random.shuffle(s)
    ...
    def _get_batch_segmentation(self, size=32, train=True, augmenter=None):
        ...
        if train:
            ...
            #indexes = [i for i in range(self.index_test, self.index_test + size)]
        ...
        random_output = [output_list[number] for number in indexes]
        ...
        for index in range(size):
            ...
            label2 = cv2.imread(random_output[index], 0)
            if events:
            ...
            if label2.shape[1] != self.width or label2.shape[0] != self.height:
                label2 = cv2.resize(label2, (self.width, self.height), interpolation=cv2.INTER_NEAREST)
        ...
        output = output.reshape((d, e, f, self.n_classes)).astype(np.uint8)

        return x, y, output, mask
...
if __name__ == "__main__":
    ...
    loader = Loader('/Volumes/SD/output-new', problemType='segmentation', n_classes=6, width=346, height=260,
                    median_frequency=0.00, channels=1, channels_events=3)
    # loader = Loader('/Volumes/Untitled/dataset_our_codification', problemType='classification', n_classes=6, width=346, height=260,
    #                 median_frequency=0.00, channels=1, channels_events=6)
    # print(loader.median_frequency_exp())
    x, y, output, mask = loader.get_batch(size=17, train=False)
    print(x.shape)
    for i in xrange(17):
        cv2.imshow('x', (x[i, :, :, 0]).astype(np.uint8))
        cv2.imwrite('./images_out/x_' + str(i) + '.jpg', (x[i, :, :, 0]).astype(np.uint8))
        cv2.imshow('1', (x[i, :, :, 1] * 127).astype(np.int8))
        cv2.imshow('2', (x[i, :, :, 2] * 127).astype(np.int8))
        cv2.imshow('3', (x[i, :, :, 3] * 127).astype(np.int8))
        y_out_out = (np.argmax(y, 3)[i, :, :]).astype(np.uint8)
        test_r = np.zeros((260, 346))
        test_g = np.zeros((260, 346))
        test_b = np.zeros((260, 346))
        test_r[np.where(y_out_out[:,:]==4)]=255
        test_g[np.where(y_out_out[:, :] == 3)] = 255
        test_b[np.where(y_out_out[:, :] == 5)] = 255

        test_r[np.where(y_out_out[:, :] == 0)] = 255
        test_b[np.where(y_out_out[:, :] == 0)] = 255

        test_r[np.where(y_out_out[:, :] == 2)] = 255
        test_g[np.where(y_out_out[:, :] == 2)] = 255

        test_r[np.where(y_out_out[:, :] == 1)] = 81
        test_g[np.where(y_out_out[:, :] == 1)] = 81
        test_b[np.where(y_out_out[:, :] == 1)] = 81

        out_put_1 = np.asarray([test_r, test_g, test_b])
        out_put_1 = out_put_1.transpose(1, 2, 0)
        y_out = (np.argmax(y, 3)[i, :, :] * 20).astype(np.uint8)
        print(out_put_1.shape)
        cv2.imshow("y",out_put_1)
        cv2.imwrite('./images_out/gt_' + str(i) + '.jpg', out_put_1)
        #utils.generate_seg_image(y_out, '', loader.dataFolderPath, loader, train=False, truth=False)
        out_out = (np.argmax(output, 3)[i, :, :] * 35).astype(np.uint8)
        out_out_out = (np.argmax(output, 3)[i, :, :]).astype(np.uint8)
        test_r = np.zeros((260, 346))
        test_g = np.zeros((260, 346))
        test_b = np.zeros((260, 346))
        test_r[np.where(out_out_out[:, :] == 4)] = 255
        test_g[np.where(out_out_out[:, :] == 3)] = 255
        test_b[np.where(out_out_out[:, :] == 5)] = 255

        test_r[np.where(out_out_out[:, :] == 0)] = 255
        test_b[np.where(out_out_out[:, :] == 0)] = 255

        test_r[np.where(out_out_out[:, :] == 2)] = 255
        test_g[np.where(out_out_out[:, :] == 2)] = 255

        test_r[np.where(out_out_out[:, :] == 1)] = 81
        test_g[np.where(out_out_out[:, :] == 1)] = 81
        test_b[np.where(out_out_out[:, :] == 1)] = 81

        out_put_1 = np.asarray([test_r, test_g, test_b])
        out_put_1 = out_put_1.transpose(1, 2, 0)
        print(out_out_out.shape)
        cv2.imshow('output',out_put_1)
        cv2.imwrite('./images_out/output_'+str(i)+'.jpg', out_put_1)
        print(mask.shape)
        cv2.imshow('mask', (mask[i, :, :] * 255).astype(np.uint8))
        cv2.waitKey(0)
    cv2.destroyAllWindows()
    x, y, output, mask = loader.get_batch(size=3, train=False)
